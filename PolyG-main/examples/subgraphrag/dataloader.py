...
class RetrieverDataset:
    ...
    def _get_triple_scores(self, dataset_name, split, processed_dict_list):
        ...
        for i in tqdm(range(len(processed_dict_list))):
            ...
            triple_scores_i, max_path_length_i = self._extract_paths_and_score(sample_i)
    ...
    def _extract_paths_and_score(self, sample):
        nx_g = self._get_nx_g(
            sample["h_id_list"], sample["r_id_list"], sample["t_id_list"]
        )
        ...
        for path in path_list_:
            ...
            max_path_length = max(max_path_length, num_triples_path)  # type: ignore
            triples_path = []
    ...
    def _assembly(
        ...
        for i in tqdm(range(len(processed_dict_list))):
            ...
            num_entities_i = len(sample_i["text_entity_list"]) + len(
                sample_i["non_text_entity_list"]
            )
            topic_entity_mask = torch.zeros(num_entities_i)
            topic_entity_mask[sample_i["q_entity_id_list"]] = 1.0
